...
def _write_parquet(
    tmp_path: Path, rows: list[dict], columns: list[str] | None = None
) -> Path:
...
def simple_parquet(tmp_path: Path) -> Path:
    ...
    rows = [
        _capex_row(
            tech="LandbasedWind", detail="Class4", case="Moderate", value=1_200_000.0
        ),
...
class TestBuildIndexCapex:
    ...
    def test_correct_capex_values_appear(self, simple_parquet: Path):
        result = build_index(simple_parquet)
        gas_row = result[
            (result["technology"] == "NaturalGas") & (result["tech_detail"] == "CT")
        ]
        wind_row = result[
            (result["technology"] == "LandbasedWind")
            & (result["tech_detail"] == "Class4")
        ]
    ...
    def test_non_positive_capex_rows_excluded(self, tmp_path: Path, bad_value: float):
        rows = [
            _capex_row(
                tech="LandbasedWind", detail="Class4", case="Moderate", value=bad_value
            ),
    ...
    def test_non_capex_parameter_rows_are_not_base_rows(self, tmp_path: Path):
        rows = [
            _capex_row(
                tech="Solar", detail="FixedTilt", case="Moderate", value=800_000.0
            ),
            # A "fixed_om_mw" row — should never appear as a base row
            {
                "data_year": 2024,
                "technology": "Solar",
                "tech_detail": "FixedTilt",
                "cost_case": "Moderate",
                "parameter": "fixed_om_mw",
                "parameter_value": 15_000.0,
            },
        ]
        p = _write_parquet(tmp_path, rows)
        result = build_index(p)
...
class TestBuildIndexHeatRate:
    ...
    def test_heat_rate_preserves_float_precision(self, tmp_path: Path):
        rows = [
            _capex_row(
                tech="NaturalGas", detail="CT", case="Moderate", value=1_500_000.0
            ),
            _heat_row(tech="NaturalGas", detail="CT", case="Moderate", value=7123.9),
        ]
        p = _write_parquet(tmp_path, rows)
        result = build_index(p)
        assert result.iloc[0]["heat_rate"] == 7123.9

    def test_heat_rate_preserves_half_values(self, tmp_path: Path):
        rows = [
            _capex_row(
                tech="NaturalGas", detail="CT", case="Moderate", value=1_500_000.0
            ),
        ...
        p = _write_parquet(tmp_path, rows)
        result = build_index(p)
        # heat_rate now stored as float to preserve precision
        assert result.iloc[0]["heat_rate"] == 7123.5

    def test_capex_only_technology_gets_heat_rate_zero_not_nan(
        self, simple_parquet: Path
    ):
        result = build_index(simple_parquet)
        wind_row = result[result["technology"] == "LandbasedWind"]
...
class TestBuildIndexColumnAliases:
    ...
    def _alias_rows(
        ...
        return [
            {
                year_col: 2024,
                tech_col: "LandbasedWind",
                detail_col: "Class4",
                case_col: "Moderate",
                "parameter": "capex_mw",
                "parameter_value": 1_200_000.0,
            },
        ]
...
class TestBuildIndexEdgeCases:
    ...
    def test_empty_parquet_returns_empty_dataframe_with_correct_columns(
        self, tmp_path: Path
    ):
        # Create a properly typed empty dataframe to avoid object dtype issues
        df = pd.DataFrame(
            columns=STD_COLS,
            data={
                "data_year": pd.Series(dtype="int64"),
                "technology": pd.Series(dtype="str"),
                "tech_detail": pd.Series(dtype="str"),
                "cost_case": pd.Series(dtype="str"),
                "parameter": pd.Series(dtype="str"),
                "parameter_value": pd.Series(dtype="float64"),
            },
        )
    ...
    def test_capex_positive_filter_parametrized(
        ...
        rows = [
            _capex_row(
                tech="TestTech", detail="TypeA", case="Moderate", value=capex_value
            )
        ]
        p = _write_parquet(tmp_path, rows)
        result = build_index(p)
