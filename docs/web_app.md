...
The generated resource-group files are automatically included in the final export: the Export step (Step 9) places them in a region-specific folder (for example `resource_groups_10r_eastern-western_nercr`) inside the downloaded settings ZIP, next to `settings/`. The `data.yml` in that ZIP lists the folder as the first `RESOURCE_GROUPS` entry (new-build wind/solar), alongside the existing resource groups. Downloading the ZIP from this tab is **optional** — use it only if you want the files standalone. See the [Downloading Input Data](#downloading-input-data) section in Step 9 for details.
...
Replace its resource path and primary data path with locations that
...
* `<resource-group folder>/` (only when resource groups were generated in Step 7) - New-build wind/solar resource-group files placed under a region-specific folder, for example `resource_groups_<regions>r_<interconnects>_<grouping>/` (such as `resource_groups_10r_eastern-western_nercr/`), next to `settings/`. The `data.yml` path template's first `RESOURCE_GROUPS` entry references this folder.
...
Below the **Download All as ZIP** button, the **Download input data** section explains where to find the input data that the generated `data.yml` references. All three required deposits are available on Zenodo; each entry lists what the deposit contains, which settings key it feeds, its Zenodo record, and a suggested local folder. `RESOURCE_GROUPS` accepts a list with two entries: the **first** is the region-specific folder for the new-build wind/solar resource groups generated by the web app in Step 7 — these files are included automatically in the Export ZIP, so no manual download or folder setup is needed; the **second** is the existing resource groups deposit — download it from Zenodo and update the path to your local copy.

The section leads with the recommended way to fetch these files: the `download_zenodo` command included in PowerGenome 0.9 and later. The deposits are not packaged as a single ZIP, so the command requests every file in a deposit, saves each collection in the suggested folder, and records the DOI and data version of each downloaded file. Because that provenance is recorded, `run_powergenome` writes a `data_sources.md` (and `.json`) citation report into each results folder listing the datasets a run used. Installs older than 0.9 can still follow the deposit links below.

```bash
download_zenodo --all            # all three collections, each to its folder below
download_zenodo data             # -> ~/PowerGenome-data/data
download_zenodo profiles         # -> ~/PowerGenome-data/resource_profiles
download_zenodo resource-groups  # -> ~/PowerGenome-data/existing_resource_groups
```

`download_zenodo --list-collections` reports the records, their default folders, and what is already on disk. Use `--dest` or `--data-root` to save somewhere else and `--force` to re-download files that already look complete.

* **PowerGenome Input Data (core)** - The 15 files referenced by `data.yml` (`reeds_generators_transformed.csv`, `plant_region_map.csv`, `technology_costs_atb.parquet`, `technology_heat_rates_nrelatb.csv`, `operational_constraints_reeds.csv`, `transmission_capacity_reeds.csv`, `fuel_prices.parquet`, `dollar_year_adjustment.csv`, `reeds_load_transformed.parquet`, `regional_cost_multipliers.csv`, `distributed_capacity.parquet`, `distributed_profiles.parquet`, `reserve_margins.csv`, `nerc_reserve_margins.csv`, `cpi_data.csv`). Feeds the `data_location` settings key. [Zenodo record 22233228](https://zenodo.org/records/22233228). Suggested folder: `~/PowerGenome-data/data`.
* **PowerGenome Renewable Resource Profiles** - Hourly capacity-factor profiles for new-build renewables. Feeds `RESOURCE_GROUP_PROFILES`. [Zenodo record 22235855](https://zenodo.org/records/22235855). Suggested folder: `~/PowerGenome-data/resource_profiles`.
* **PowerGenome Existing Renewable Resource Groups** - Resource group metadata for existing renewable generators. The second `RESOURCE_GROUPS` entry points to this deposit's folder; the first entry is the region-specific folder for the new-build wind/solar resource groups generated in Step 7 and included automatically in the Export ZIP. [Zenodo record 22235662](https://zenodo.org/records/22235662). Suggested folder: `~/PowerGenome-data/existing_resource_groups`.

The section also includes a copy-paste-ready `data.yml` snippet pointing each setting at its suggested folder. The snippet reflects the current **Region Name** entered in Step 7: the first `RESOURCE_GROUPS` entry uses that name (the region-specific folder included in the Export ZIP), so it updates live if you change the Region Name. The second `RESOURCE_GROUPS` entry points to the existing resource groups deposit — download it from Zenodo and update the path to your local copy.

```yaml
data_location: ["~/PowerGenome-data/data"]
RESOURCE_GROUPS:
  - "resource_groups_10r_eastern-western_nercr"  # new-build groups, in the export ZIP
  - "~/PowerGenome-data/existing_resource_groups"  # existing groups (Zenodo)
...
The same instructions are written to `DATA_SOURCES.md` at the root of the **Download All** ZIP so they travel with your settings. The section is rendered from a single `DATA_SOURCES` configuration block in `web/cluster_app.py`; the three production Zenodo deposits listed above are available for download.
...
When generated wind or solar LCOE Parquet files are included under the region-specific
resource-groups folder (for example `resource_groups_10r_eastern-western_nercr/`), the
manifest also omits the matching source tables and
...
required_supplemental_files:
  - resource_groups_10r_eastern-western_nercr/my_region_groups.json   # listed only when supplemental files exist
forms: { ... }
state: { ... }
tables: { ... }
```
