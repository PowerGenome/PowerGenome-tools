...
Below the **Download All as ZIP** button, the **Download input data** section explains where to find the input data that the generated `data.yml` references. Three Zenodo deposits are documented; each entry lists what the deposit contains, which settings key it feeds, its Zenodo link/DOI, and a suggested local folder. `RESOURCE_GROUPS` accepts a list with two entries: the **first** is the region-specific folder for the new-build wind/solar resource groups generated by the web app in Step 7 — these files are included automatically in the Export ZIP, so no manual download or folder setup is needed; the **second** is the existing resource groups deposit — download it from Zenodo and update the path to your local copy.
...
* **PowerGenome Existing Renewable Resource Groups** - Resource group metadata for existing renewable generators. The second `RESOURCE_GROUPS` entry points to this deposit's folder; the first entry is the region-specific folder for the new-build wind/solar resource groups generated in Step 7 and included automatically in the Export ZIP. Zenodo link and DOI are placeholders until the deposit is published. Suggested folder: `~/PowerGenome-data/existing_resource_groups`.
