...
Downloads from this tab are provided as a single ZIP archive. Save the downloaded resource-group files to a local folder and reference that folder in the generated settings: the Export step's `data.yml` lists it as the second `RESOURCE_GROUPS` entry (new-build wind/solar), alongside the existing resource groups. See the [Downloading Input Data](#downloading-input-data) section in Step 9 for the recommended folder layout.
...
* **PowerGenome Existing Renewable Resource Groups** - Resource group metadata for existing renewable generators. One of the folders in the `RESOURCE_GROUPS` list; the second folder holds the new-build wind/solar resource groups generated by the web app in Step 7. Zenodo link and DOI are placeholders until the deposit is published. Suggested folder: `~/PowerGenome-data/existing_resource_groups`.

The section also includes a copy-paste-ready `data.yml` snippet pointing each setting at its suggested folder. The second `RESOURCE_GROUPS` folder is where you save the resource group files downloaded from Step 7 (Interconnection); the first entry is the existing resource groups deposit.
