...
Downloads from this tab are provided as a single ZIP archive. Save the downloaded resource-group files to a folder in your project (for example `resource_groups`, next to `data.yml`) and reference that folder in the generated settings: the Export step's `data.yml` lists it as the second `RESOURCE_GROUPS` entry (new-build wind/solar), alongside the existing resource groups. See the [Downloading Input Data](#downloading-input-data) section in Step 9 for the recommended folder layout.
...
The section also includes a copy-paste-ready `data.yml` snippet pointing each setting at its suggested folder. The second `RESOURCE_GROUPS` folder is where you save the resource group files downloaded from Step 7 (Interconnection); the first entry is the existing resource groups deposit. Keep this second folder in your project (for example `resource_groups`, next to `data.yml`) rather than in `~/PowerGenome-data`.

```yaml
data_location: ["~/PowerGenome-data/data"]
RESOURCE_GROUPS:
  - "~/PowerGenome-data/existing_resource_groups"
  - "resource_groups"  # new-build groups, in your project folder
