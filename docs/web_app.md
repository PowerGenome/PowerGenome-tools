...
Downloads from this tab are provided as a single ZIP archive. Unzip the downloaded resource-group files into a folder and place it next to the `settings` folder (for example `resource_groups` beside `settings`), then reference that folder in the generated settings: the Export step's `data.yml` lists it as the second `RESOURCE_GROUPS` entry (new-build wind/solar), alongside the existing resource groups. See the [Downloading Input Data](#downloading-input-data) section in Step 9 for the recommended folder layout.
...
Below the **Download All as ZIP** button, the **Download input data** section explains where to find the input data that the generated `data.yml` references. Three Zenodo deposits are documented; each entry lists what the deposit contains, which settings key it feeds, its Zenodo link/DOI, and a suggested local folder. `RESOURCE_GROUPS` accepts a list of folders: the existing resource groups deposit plus the new-build wind/solar resource groups generated by the web app (Step 7). Keep this second `RESOURCE_GROUPS` folder in your project, next to the `settings` folder (for example `resource_groups` beside `settings`), rather than in `~/PowerGenome-data`.
...
* **PowerGenome Existing Renewable Resource Groups** - Resource group metadata for existing renewable generators. One of the folders in the `RESOURCE_GROUPS` list; the second folder holds the new-build wind/solar resource groups generated by the web app in Step 7 (kept in your project folder). Zenodo link and DOI are placeholders until the deposit is published. Suggested folder: `~/PowerGenome-data/existing_resource_groups`.

The section also includes a copy-paste-ready `data.yml` snippet pointing each setting at its suggested folder. The snippet reflects the current **Region Name** entered in Step 7: the second `RESOURCE_GROUPS` entry uses that name, so it updates live if you change the Region Name. The second `RESOURCE_GROUPS` folder is where you save the resource group files downloaded from Step 7 (Interconnection); the first entry is the existing resource groups deposit. Keep this second folder in your project, next to the `settings` folder (for example `resource_groups` beside `settings`), rather than in `~/PowerGenome-data`.

```yaml
data_location: ["~/PowerGenome-data/data"]
RESOURCE_GROUPS:
  - "~/PowerGenome-data/existing_resource_groups"
  - "resource_groups"  # new-build groups, next to settings
